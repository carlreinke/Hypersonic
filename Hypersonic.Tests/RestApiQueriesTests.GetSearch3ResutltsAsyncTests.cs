...
namespace Hypersonic.Tests
{
    ...
    {
        public static class GetSearch3ResutltsAsyncTests
        {
            [Fact]
            public static void TestGetSearch3ResutltsAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetSearch3ResultsAsync(dbContext, user.UserId, null, artist.Name, 0, 10, 0, 10, 0, 10, "opus", CancellationToken.None).Result;
