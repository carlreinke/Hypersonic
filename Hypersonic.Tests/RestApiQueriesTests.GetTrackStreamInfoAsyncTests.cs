...
namespace Hypersonic.Tests
{
    ...
    {
        public static class GetTrackStreamInfoAsyncTests
        {
            [Fact]
            public static void TestGetTrackStreamInfoAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetTrackStreamInfoAsync(dbContext, user.UserId, track.TrackId, CancellationToken.None).GetAwaiter().GetResult();
