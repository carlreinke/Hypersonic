...
namespace Hypersonic.Tests
{
    ...
    {
        public static class GetRandomSongsAsyncTests
        {
            [Fact]
            public static void TestGetRandomSongsAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetRandomSongsAsync(dbContext, user.UserId, null, null, null, null, 10, "opus", CancellationToken.None).Result;
