...
namespace Hypersonic.Tests
{
    public static partial class RestApiQueriesTests
    {
        ...
        {
            [Fact]
            public static void TestGetAlbumList2StarredAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetAlbumList2StarredAsync(dbContext, user.UserId, null, 0, 10, CancellationToken.None).Result;
