...
namespace Hypersonic.Tests
{
    public static partial class RestApiQueriesTests
    {
        ...
        {
            [Fact]
            public static void TestGetAlbumList2ByYearAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetAlbumList2ByYearAsync(dbContext, user.UserId, null, 0, 10, album.Date.Value / 10000, album.Date.Value / 10000, CancellationToken.None).Result;
