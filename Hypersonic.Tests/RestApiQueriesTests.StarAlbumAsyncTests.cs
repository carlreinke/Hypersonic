...
namespace Hypersonic.Tests
{
    ...
    {
        public static class StarAlbumAsyncTests
        {
            [Fact]
            public static void TestStarAlbumAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    for (int i = 0; i < 2; ++i)
                    {
                        RestApiQueries.StarAlbumAsync(dbContext, user.UserId, album.AlbumId, CancellationToken.None).GetAwaiter().GetResult();
                        dbContext.SaveChanges();

                        Assert.True(dbContext.AlbumStars.Any(s => s.AlbumId == album.AlbumId && s.UserId == user.UserId));
                    }
                }
            }
        }
