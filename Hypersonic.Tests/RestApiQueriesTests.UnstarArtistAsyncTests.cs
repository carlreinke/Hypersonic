...
namespace Hypersonic.Tests
{
    ...
    {
        public static class UnstarArtistAsyncTests
        {
            [Fact]
            public static void TestUnstarArtistAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    for (int i = 0; i < 2; ++i)
                    {
                        RestApiQueries.UnstarArtistAsync(dbContext, user.UserId, artist.ArtistId, CancellationToken.None).Wait();
                        dbContext.SaveChanges();
