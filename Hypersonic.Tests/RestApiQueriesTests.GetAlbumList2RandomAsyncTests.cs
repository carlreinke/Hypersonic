...
namespace Hypersonic.Tests
{
    public static partial class RestApiQueriesTests
    {
        ...
        {
            [Fact]
            public static void TestGetAlbumList2RandomAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetAlbumList2RandomAsync(dbContext, user.UserId, null, 10, CancellationToken.None).Result;
