...
namespace Hypersonic.Tests
{
    public static partial class RestApiQueriesTests
    {
        ...
        {
            [Fact]
            public static void TestGetAlbumList2OrderedByAlbumTitleAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetAlbumList2OrderedByAlbumTitleAsync(dbContext, user.UserId, null, 0, 10, CancellationToken.None).Result;
