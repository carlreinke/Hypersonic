...
namespace Hypersonic.Tests
{
    public static partial class RestApiQueriesTests
    {
        ...
        {
            [Fact]
            public static void TestAddPlaylistSongsAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    RestApiQueries.AddPlaylistSongsAsync(dbContext, user.UserId, playlist.PlaylistId, new[] { track2.TrackId }, CancellationToken.None).Wait();
                    dbContext.SaveChanges();

                    Assert.True(dbContext.PlaylistTracks.Any(pt => pt.PlaylistId == playlist.PlaylistId && pt.TrackId == track1.TrackId));
