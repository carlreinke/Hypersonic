...
namespace Hypersonic.Tests
{
    ...
    {
        public static class GetSongAsyncTests
        {
            [Fact]
            public static void TestGetSongAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetSongAsync(dbContext, user.UserId, track.TrackId, "opus", CancellationToken.None).GetAwaiter().GetResult();
