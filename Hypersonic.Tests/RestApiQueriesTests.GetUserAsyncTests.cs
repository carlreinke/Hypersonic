...
namespace Hypersonic.Tests
{
    ...
    {
        public static class GetUserAsyncTests
        {
            [Fact]
            public static void TestGetUserAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    var result = RestApiQueries.GetUserAsync(dbContext, user.Name, CancellationToken.None).Result;
