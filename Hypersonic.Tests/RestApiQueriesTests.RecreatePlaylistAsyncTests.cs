...
namespace Hypersonic.Tests
{
    ...
    {
        public static class RecreatePlaylistAsyncTests
        {
            [Fact]
            public static void TestRecreatePlaylistAsync()
            {
                ...
                using (var dbContext = new MediaInfoContext(dbContextOptionsBuilder.Options))
                {
                    ...
                    RestApiQueries.RecreatePlaylistAsync(dbContext, user.UserId, playlist.PlaylistId, "playlistName", CancellationToken.None).GetAwaiter().GetResult();
                    _ = dbContext.SaveChanges();
